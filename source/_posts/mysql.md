...
使用索引扫描来做排序
1.mysql可以使用同一个索引，既满足排序又可用于查找行，设计索引时尽可能同时满足这两个条件，这样是最好的
2.只有当索引的列顺序和order by字句的顺序完全一致，并且所有列的排序方向都一样时，才可以使用索引来做排序
3.order by 列顺序也要遵循最左前缀原则
```

#### 查询性能优化
``` 
慢查询出现时思考方向
1.是否向数据库请求了不需要的数据，尽量避免 SELECT *  无法使用覆盖索引
2.mysql是否在扫描额外的记录【响应时间，扫描的函数和返回的行数，扫描的行数和访问类型】
...
查询执行的过程
1.客户端发送一条sql给服务端
2.服务器先检查查询缓存，如果命中了缓存，就立刻返回结果给客户端，否则进入下一步
